        ...
        try {
            ...
            if (context.alertType === 'standard') {
                if (context.interaction) {
                    await context.interaction.author.send(`Alert: ${context.formattedPair} has moved ${context.direction} to target ${context.targetPrice}. Current price: ${newPrice}. Type: ${context.alertType} Time: ${timestamp}.`);
                }
                ...
                if (context.interaction) {
                    await context.interaction.author.send(`Alert: ${context.formattedPair} has crossed ${context.targetPrice}. Current price: ${newPrice}. Type: ${context.alertType}Time: Time: ${timestamp}.`);
                }
                console.log(`Alert ${alertId} notification sent to the user.`);
            ...
            context.message.channel.send('There was an error setting up your alert. Please try again.')
                .catch(err => console.error(`Failed to send error message to user ID: ${context.interaction.author.id}`, err));
        }
    }
}
